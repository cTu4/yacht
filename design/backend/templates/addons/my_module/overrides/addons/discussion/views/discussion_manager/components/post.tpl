...
<div class="d-flex f-wrap">
    {foreach from=$post.ratings item="rating" key="key"}
        <div class="control-group d-flex align-baseline w-30 j-content-f-end">
            <label for="{$key}" class="label-start">{__($key)}: </label>
            <div id="{$key}" class="d-flex">
                {include file="addons/discussion/views/discussion_manager/components/stars.tpl" stars=$rating}
            </div>
        </div>
    {/foreach}
</div>
